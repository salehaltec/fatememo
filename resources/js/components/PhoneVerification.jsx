...
export default function PhoneVerification({ assessment, onVerified, accent = '#2563eb', dark = false, purpose = 'result' }) {
  ...
  const verify = async () => { setBusy(true); setError(''); try {
    ...
    if (assessment) await api('/api/assessments',{method:'POST',body:JSON.stringify(assessment)});
  ...
  return <div style={{width:'100%',minHeight:'calc(100dvh - 5rem)',display:'grid',placeItems:'center',padding:'20px 0',boxSizing:'border-box'}}><div style={{position:'relative',width:'100%',maxWidth:540,margin:'0 auto',padding:'1px',borderRadius:24,background:`linear-gradient(135deg,${accent}aa,rgba(255,255,255,.18),${accent}35)`,boxShadow:`0 24px 80px ${accent}20,0 14px 44px rgba(0,0,0,.2)`,fontFamily:'Vazirmatn, sans-serif',overflow:'hidden'}}>
    ...
    <div style={{display:'inline-flex',alignItems:'center',gap:7,color:accent,fontWeight:800,fontSize:12,letterSpacing:'.02em',marginBottom:8}}><span style={{width:7,height:7,borderRadius:'50%',background:accent,boxShadow:`0 0 12px ${accent}`}}/> {purpose === 'contact' ? 'ثبت درخواست جلسه' : 'تحلیل کسب و کار شما انجام شد!'}</div>
    <h2 style={{margin:'0 0 9px',fontSize:'clamp(20px,4vw,27px)',lineHeight:1.5}}>تایید شماره همراه</h2>
    <p style={{color:muted,lineHeight:1.9,fontSize:14,margin:'0 0 18px'}}>{purpose === 'contact' ? 'برای ثبت درخواست جلسه آشنایی، ابتدا شماره موبایل خود را با کد یک‌بارمصرف تایید کنید.' : 'برای حفظ محرمانگی اطلاعات و مشاهده تحلیل اختصاصی، شماره موبایل خود را با کد یک‌بارمصرف تایید کنید.'}</p>
